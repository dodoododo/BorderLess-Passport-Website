...
import {
  ...
  Stethoscope, BedDouble, Biohazard, Building2, Map
} from 'lucide-react';
// ============================================================================
// 1. RAW CSS
// ============================================================================
...
const getReligionBg = (name: string) => {
  ...
  if (lower.includes('christian')) return '/religion-img/christian.jpg'; // Cơ Đốc giáo chung
...
interface DiscoverData {
  ...
  details: {
    ...
    military: { expenditures: number | null; serviceAge: number | null; transnationalIssues: string[] };
...
export default function CountryDiscover() {
        ...
        {filteredCountries.length > 0 && (
          <div className="w-full grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 auto-rows-max">
            {filteredCountries.map((c) => (
              <button
                key={c.code}
                onClick={() => setSelectedIso2(c.code)}
                className="relative aspect-[4/5] sm:aspect-square overflow-hidden group cursor-pointer focus:outline-none bg-slate-900"
              >
                <div 
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-[#F4F1EA] flex flex-col items-center justify-center text-center p-6">
        ...
        <button onClick={() => setSelectedIso2(null)} className="px-6 py-2 bg-slate-900 text-white font-data text-sm rounded-lg hover:bg-slate-800">RETURN</button>
      </div>
    );
  }
  ...
  const accentVars: CustomCSSProperties = {
    ...
    "--cm-accent-soft": core.flag?.accentSoft || hexToRgba(rawColor, 0.09),
  ...
  return (
    ...
    >
      ...
      <header className="relative mx-auto pb-8">
        ...
        */}
        <div className="relative w-full overflow-hidden shadow-xl bg-slate-900 min-h-[46vh] md:min-h-[58vh] flex items-end p-8 pt-28 md:p-14 md:pt-20 group">
          ...
          {d.coverImageUrl && (
            <img src={d.coverImageUrl} alt="Cover" className="absolute inset-0 w-full h-full object-cover opacity-80 group-hover:opacity-90 group-hover:scale-101 transition-all duration-1000" />
          )}
          ...
          <div className="relative z-10 w-full flex items-end justify-between flex-wrap gap-8">
            <div className="flex-1">
              ...
              <h2 className="font-display italic text-2xl text-white/80 max-w-2xl drop-shadow-md">
                {Array.from(new Set([core.nativeOfficial, core.nameOfficial].filter(Boolean))).join(' • ')}
              </h2>
              <h3 className="font-display text-xl text-white/80 max-w-2xl drop-shadow-md mt-3">
                {d.etymology}
              </h3>
            </div>
            ...
            <div className="flex flex-col items-end shrink-0 max-w-2xl">
              <div className="mb-4 h-full shrink-0 rounded-[1px] overflow-hidden border border-foreground/100 shadow-sm flex items-center justify-center">
                <span className={`fi fi-${core.iso2.toLowerCase()} block text-[28px] sm:text-[150px] leading-none !bg-cover !bg-center`} />
              </div>
      ...
      <div className="mx-auto mb-14">
        ...
        <div className="w-full mb-9 px-4 sm:px-6 ">
          <div className="w-full bg-white/60 backdrop-blur-sm p-8 md:p-6 shadow-sm relative overflow-hidden border border-slate-200/80">
            {/* Dải màu trang trí tinh tế phía trên khung */}
            {/* DÒNG TIÊU ĐỀ TRƯỚC KHI VÀO NỘI DUNG */}
            ...
            {/* Thêm [column-rule:...]: Tự động tạo đường gạch thẳng mờ giữa các cột */}
            <div className="font-editorial text-lg md:text-base text-slate-700 leading-[1.8] columns-1 md:columns-2 lg:columns-3 gap-10 [column-rule:2px_solid_rgba(0,0,0,0.3)] [column-fill:_balance]">
              {/* Drop Cap cho ký tự đầu tiên */}
              <span className="float-left text-5xl md:text-7xl font-display font-black text-accent mr-4 leading-none select-none">
                {d.introduction ? d.introduction.charAt(0) : ''}
              </span>
              {/* Phần nội dung còn lại */}
              {d.introduction ? d.introduction.slice(1) : ''}
            </div>
          </div>
        </div>
              ...
              {core.currencies?.length > 0 ? (
                core.currencies.map((c, index) => (
                  <span key={index} className="block">
                    ...
                    {c.symbol && (
                      <span className="inline-flex items-center justify-center w-8 h-8 shrink-0 ml-2 rounded-full bg-white text-accent text-[16px] font-bold shadow-[0_2px_8px_-2px_rgba(0,0,0,0.5)]">
                        {c.symbol}
                      </span>
                    )}
                  </span>
      ...
      <div className="mx-auto px-4 sm:px-6 space-y-14">

        {/* GEOGRAPHY & DEMOGRAPHICS */}
        <div>
          <SectionHeader title="Geography & Demographics" icon="geo" />

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
            {/* Environment & Terrain */}
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
              <h3 className="font-display text-lg font-bold text-slate-800 uppercase tracking-wide mb-5 flex items-center gap-2">
                {/* Vạch màu trang trí nhẹ nhàng thay cho icon/chữ khổng lồ */}
                <span className="w-1 h-5 bg-accent-raw rounded-sm"></span>
                Environment & Terrain
              </h3>
              
              {/* Bọc MiniRow vào space-y-1 để khoảng cách dòng đều tăm tắp */}
              <div className="space-y-1">
                <MiniRow label="Climate" value={d.geography?.climate} />
                <MiniRow label="Terrain" value={d.geography?.terrain} />
                <MiniRow label="Coastline" value={d.geography?.coastlineKm ? `${d.geography.coastlineKm.toLocaleString()} km` : undefined} />
                <MiniRow label="Highest Point" value={d.geography?.elevation?.highestPoint} suffix={d.geography?.elevation?.highestElevation ? `(${d.geography.elevation.highestElevation}m)` : ''} />
                <MiniRow label="Lowest Point" value={d.geography?.elevation?.lowestPoint} suffix={d.geography?.elevation?.lowestElevation ? `(${d.geography.elevation.lowestElevation}m)` : ''} />
                <MiniRow label="Land Borders" value={core.borders?.length ? core.borders.join(', ') : (core.landlocked ? 'Landlocked' : 'Island / No Borders')} />
              </div>

              <div className="mt-5 pt-5 border-t border-slate-100">
                <span className="block font-data text-[10px] uppercase tracking-widest text-slate-700 mb-3 font-semibold">Natural Hazards</span>
                <div className="flex flex-wrap gap-2">
                  {d.geography?.naturalHazards?.map((h, i) => (
                    <Tag key={i} text={h} color="bg-slate-50 text-slate-600 border border-slate-200" />
                  ))}
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
              ...
              <div className="flex w-full gap-4 mb-6">
                ...
                {/* Cột 1: Total Population */}
                <div className="flex-1 border-l-[3px] border-slate-200 pl-4 py-0.5 overflow-hidden">
                  <span className="font-data text-[10px] uppercase tracking-widest text-slate-500 block mb-1 truncate">Total Population</span>
                  <span className="font-ui text-2xl font-bold text-slate-800 block truncate">
                    {core.population ? (core.population / 1e6).toFixed(1) + 'M' : '—'}
                  </span>
                </div>

                {/* Cột 2: Growth Rate - Đã đổi border và màu chữ y chang bên trái */}
                <div className="flex-1 border-l-[3px] border-slate-200 pl-4 py-0.5 overflow-hidden">
                  <span className="font-data text-[10px] uppercase tracking-widest text-slate-500 block mb-1 truncate">Growth Rate</span>
                  <span className="font-ui text-2xl font-bold text-slate-800 block truncate">
                    {d.demographics?.populationGrowthRate}%
                  </span>
                </div>
              ...
              <div className="space-y-3">
                <ProgressBar label="0-14 years" percent={d.demographics?.ageStructure?.["0_14"]} />
                <ProgressBar label="15-24 years" percent={d.demographics?.ageStructure?.["15_24"]} />
                <ProgressBar label="25-54 years" percent={d.demographics?.ageStructure?.["25_54"]} />
                <ProgressBar label="55-64 years" percent={d.demographics?.ageStructure?.["55_64"]} />
                <ProgressBar label="65+ years" percent={d.demographics?.ageStructure?.["65_over"]} />
              </div>
            </div>
          </div>

          {/* Bottom Row - DataBoxes */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            <DataBox title="Health & Education">
              <div className="space-y-1">
                <MiniRow label="Life Expectancy" value={d.demographics?.lifeExpectancy ? `${d.demographics.lifeExpectancy} yrs` : undefined} />
                <MiniRow label="Literacy Rate" value={d.demographics?.literacyRate ? `${d.demographics.literacyRate}%` : undefined} />
                <MiniRow label="School Life Expect." value={d.demographics?.schoolLifeExpectancy ? `${d.demographics.schoolLifeExpectancy} yrs` : undefined} />
                <MiniRow label="Median Age" value={d.demographics?.medianAge ? `${d.demographics.medianAge} yrs` : undefined} />
                <MiniRow label="Mean Maternal Age" value={d.demographics?.meanMaternalAge ? `${d.demographics.meanMaternalAge} yrs` : undefined} />
                <MiniRow label="Obesity Rate" value={d.demographics?.obesityRate ? `${d.demographics.obesityRate}%` : undefined} />
                <MiniRow label="Sex Ratio" value={d.demographics?.sexRatioTotal ? `${d.demographics.sexRatioTotal} male per female` : undefined} />
              </div>
            </DataBox>

            <DataBox title="Medical Infrastructure">
              <div className="space-y-1">
                <MiniRow label="Physicians / 1k" value={d.demographics?.physiciansDensity} />
                <MiniRow label="Hospital Beds / 1k" value={d.demographics?.hospitalBedDensity} />
              </div>
              {/* Bỏ khối màu đỏ lè, thay bằng warning gạch dọc tinh tế */}
              <div className="mt-6 border-l-[3px] border-red-500 pl-4 py-1">
                <strong className="block font-data text-[10px] uppercase tracking-widest mb-1 text-red-600">Infectious Disease Risk</strong>
                <span className="font-ui text-sm font-semibold text-slate-800">{d.demographics?.infectiousDiseasesRisk || 'None or unspecified'}</span>
              </div>
            </DataBox>

            <DataBox title="Ethnicities & Urbanization">
              <MiniRow label="Urbanization Rate" value={d.demographics?.urbanizationRate ? `${d.demographics.urbanizationRate}%` : undefined} />
              
              <div className="mt-5 mb-5">
                <span className="block font-data text-[10px] uppercase tracking-widest text-slate-500 mb-3 font-semibold">Ethnic Groups</span>
                {d.demographics?.ethnicGroups?.length > 0 ? (
                  <div className="space-y-2">
                    {d.demographics.ethnicGroups.map((e, i) => <ProgressBar key={i} label={e.name} percent={e.percent} />)}
                  </div>
                ) : <span className="text-sm text-slate-400">N/A</span>}
              </div>
              
              <div className="mt-5 pt-5 border-t border-slate-100">
                <span className="block font-data text-[10px] uppercase tracking-widest text-slate-500 mb-3 font-semibold">Major Urban Areas</span>
                <ul className="space-y-2 text-sm font-ui text-slate-700">
                  {d.demographics?.majorUrbanAreas?.map((u, i) => (
                    <li key={i} className="flex justify-between items-center group">
                      ...
                      <span className="font-bold text-slate-800">{(u.population/1e6).toFixed(1)}M</span>
                    </li>
                  ))}
                </ul>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3">
                ...
                <div>
                  ...
                  <span className="font-ui text-base font-medium text-slate-800 inline-flex items-center gap-1.5">
                    ...
                    {d.geography?.coastlineKm ? `${d.geography.coastlineKm.toLocaleString()} km` : '0 km (Landlocked)'}
                  </span>
                </div>
                <div>
                  <span className="block font-data text-base capitalize text-slate-500 mb-1">Borders</span>
                  <span
                    className="font-ui text-base text-slate-700 leading-snug line-clamp-2"
                    title={core.borders?.join(', ')}
                  >
                    {core.borders?.length ? core.borders.join(', ') : (core.landlocked ? 'Landlocked' : 'Island / No Borders')}
                  </span>
                </div>
                ...
                {[
                  ...
                  <div key={i} className="flex justify-between items-end py-2.5 border-b border-slate-200 last:border-0 group">
                    ...
                    <span className="text-[15px] font-bold text-slate-800">
                      ...
                      {item.value && <span className="text-[13px] font-bold text-slate-600 ml-0.5">{item.suffix}</span>}
                    </span>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-white rounded-lg p-7 border border-slate-200 shadow-sm hover:shadow-md transition-shadow flex flex-col">
              ...
              <div className="mt-auto bg-rose-50/50 border border-rose-100 rounded-md p-4">
                <div className="flex items-center gap-2 mb-2">
                  <Biohazard className="w-3.5 h-3.5 text-rose-500" strokeWidth={2} />
                  <strong className="text-[10px] uppercase tracking-widest font-bold text-rose-600">
                    Disease Risk
                  </strong>
                </div>
                <span className="text-[12px] font-medium text-slate-700 leading-relaxed block">
        ...
        <div className="mb-14">
          ...
          <div className="w-full bg-white/60 backdrop-blur-sm p-8 md:p-6 shadow-sm relative overflow-hidden border border-slate-200/80 mb-8">
            ...
            <div className="font-editorial text-lg md:text-base text-slate-700 leading-[1.8] columns-1 md:columns-2 lg:columns-3 gap-10 [column-rule:2px_solid_rgba(0,0,0,0.4)] [column-fill:_balance]">
              {/* Drop Cap cách điệu cho chữ cái đầu tiên của đoạn văn kinh tế */}
              <span className="float-left text-5xl md:text-7xl font-display font-black text-accent mr-4 leading-none select-none italic">
                {d.economy?.overview ? d.economy.overview.charAt(0) : ''}
              </span>
              {/* Phần nội dung kinh tế còn lại */}
              {d.economy?.overview ? d.economy.overview.slice(1) : ''}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            <div className="bg-white p-7 rounded-3xl shadow-sm border border-slate-100 flex flex-col">
              ...
              <div className="mt-4 pt-4 border-t border-slate-100">
                <span className="block font-data text-[10px] uppercase text-slate-500 mb-2">Industries</span>
                <div className="flex flex-wrap gap-2 mb-4">{d.economy?.industries?.map((ind, i) => <Tag key={i} text={ind} />)}</div>

                <span className="block font-data text-[10px] uppercase text-slate-500 mb-2">Agriculture</span>
                <div className="flex flex-wrap gap-2">{d.economy?.agricultureProducts?.map((ag, i) => <Tag key={i} text={ag} color="bg-accent-raw text-accent" />)}</div>
              </div>

              <div className="mt-auto pt-5 grid grid-cols-2 gap-4">
                <div>
                  <span className="block font-data text-[10px] uppercase text-slate-500 mb-2">Top Exports To</span>
                  {d.economy?.exportPartners?.slice(0,3).map((p, i) => <ProgressBar key={i} label={p.name} percent={p.percent} />)}
                </div>
                <div>
                  <span className="block font-data text-[10px] uppercase text-slate-500 mb-2">Top Imports From</span>
                  {d.economy?.importPartners?.slice(0,3).map((p, i) => <ProgressBar key={i} label={p.name} percent={p.percent} />)}
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-7 rounded-3xl shadow-sm border border-slate-100 flex flex-col">
              ...
              <MiniRow label="Gov Type" value={d.government?.type} />
              <MiniRow label="Independence" value={d.government?.independenceDate} />
              <MiniRow label="Voting Age" value={d.government?.civics?.votingAge ? `${d.government.civics.votingAge} yrs` : undefined} />
              ...
              <div className="mt-4 pt-4 border-t border-slate-100 space-y-3">
                <div><span className="block font-data text-[10px] uppercase text-slate-500">Executive</span><p className="font-ui text-sm line-clamp-2">{d.government?.executiveBranch}</p></div>
                <div><span className="block font-data text-[10px] uppercase text-slate-500">Legislative</span><p className="font-ui text-sm line-clamp-2">{d.government?.legislativeBranch}</p></div>
                <div><span className="block font-data text-[10px] uppercase text-slate-500">Judicial</span><p className="font-ui text-sm line-clamp-2">{d.government?.judicialBranch}</p></div>
              </div>

              <div className="mt-5 pt-4 border-t border-slate-100">
                <span className="block font-data text-[10px] uppercase text-slate-500 mb-2">National Holidays</span>
                <ul className="text-sm font-ui space-y-1 h-32 overflow-y-auto pr-2">
            ...
            <div className="bg-white p-7 rounded-3xl shadow-sm border border-slate-100 flex flex-col">
              <h3 className="font-display text-2xl text-accent mb-5">Infrastructure & Military</h3>
              ...
              <div className="mt-auto pt-5">
                <div className="bg-accent-raw text-white p-5 rounded-2xl shadow-[inset_0_1px_0_rgba(255,255,255,0.15)]">
                  <h4 className="font-data text-[10px] uppercase text-white/70 mb-3">Defense & Transnational Issues</h4>
                  <div className="flex justify-between border-b border-white/20 pb-2 mb-2 font-ui text-sm">
                    <span className="text-white/90">Expenditures</span>
                    <span className="font-bold">{d.military?.expenditures ? `${d.military.expenditures}% GDP` : 'N/A'}</span>
                  </div>
                  <div className="flex justify-between border-b border-white/20 pb-2 mb-3 font-ui text-sm">
                    <span className="text-white/90">Service Age</span>
                    <span className="font-bold">{d.military?.serviceAge ? `${d.military.serviceAge} yrs` : 'N/A'}</span>
                  </div>
                  <p className="font-ui text-xs text-white">
                    {d.military?.transnationalIssues?.length ? d.military.transnationalIssues.join(' • ') : 'No major transnational issues reported.'}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="relative py-14">
          <div className="absolute inset-0 bg-white rounded-[2.5rem] shadow-xl z-0" />
          <div className="relative z-10 px-6 lg:px-16">

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-16">
              <div>
                <h3 className="font-display italic text-3xl text-accent mb-5">The National Vibe</h3>
                <p className="font-editorial text-xl text-slate-700 leading-[1.8] drop-cap mb-7">
                  {d.culturalNuances?.nationalVibe}
                </p>
                <div className="bg-slate-50 p-6 rounded-3xl border border-slate-200">
                  <h4 className="font-data text-sm tracking-widest uppercase text-slate-500 mb-3">Communication Style</h4>
                  <p className="font-ui text-slate-800 leading-relaxed">{d.culturalNuances?.communicationStyle}</p>
                </div>
              </div>

              <div className="bg-accent-raw text-white p-7 md:p-9 rounded-[2rem] shadow-xl relative overflow-hidden">
                <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full blur-3xl" />
                <h3 className="font-display text-3xl mb-7 relative z-10">Unwritten Rules (Etiquette)</h3>
                <div className="space-y-7 relative z-10">
                  <div>
                    <h4 className="font-data text-md tracking-widest text-green-100 mb-4 bg-green-800 border border-green-500 inline-block px-2 py-1">DO'S (EMBRACE)</h4>
                    <ul className="space-y-3">
                      {d.culturalNuances?.etiquette?.dos?.map((item, i) => (
                        <li key={i} className="flex gap-3 font-ui text-lg text-green-100 leading-relaxed"><span className="text-green-400">●</span> {item}</li>
                      ))}
                    </ul>
                  </div>
                  <div className="h-px bg-white/20" />
                  <div>
                    <h4 className="font-data text-md tracking-widest text-white mb-4 bg-red-800 border border-red-500 inline-block px-2 py-1 rounded">DON'TS (AVOID)</h4>
                    <ul className="space-y-3">
                      {d.culturalNuances?.etiquette?.donts?.map((item, i) => (
                        <li key={i} className="flex gap-3 font-ui text-lg text-red-100 leading-relaxed"><span className="text-red-400">■</span> {item}</li>
                      ))}
                    </ul>
                  </div>
                </div>
            ...
            <div className="mb-20">
              <h3 className="font-display text-4xl text-slate-900 mb-9 text-center">Historical Timeline</h3>
              <div className="max-w-4xl mx-auto relative timeline-line py-4">
                {d.historyTimeline?.map((item, i) => (
                  <div key={i} className="relative pl-12 py-5 group">
                    <div className="absolute left-[10.5px] top-7 w-3 h-3 bg-white border-2 border-accent rounded-full group-hover:bg-accent transition-colors shadow-sm" />
                    <div className="bg-[#F4F1EA] p-6 rounded-3xl border border-slate-200 shadow-sm group-hover:shadow-md group-hover:border-accent/30 transition-all">
                      <span className="font-data text-accent font-bold text-lg mb-2 block">{item.year}</span>
                      <h4 className="font-display font-bold text-2xl text-slate-900 mb-2">{item.event}</h4>
                      <p className="font-editorial text-[17px] text-slate-700 leading-relaxed">{item.description}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                {d.mustVisitPlaces?.map((place, i) => (
                  <div key={i} className="group cursor-pointer">
                    <div className="w-full h-72 rounded-3xl overflow-hidden mb-4 relative shadow-sm group-hover:shadow-2xl transition-all duration-500 transform group-hover:-translate-y-2">
                      {place.imageUrl ? (
                      ...
                      <div className="absolute inset-0 bg-gradient-to-t from-[color:var(--cm-accent-raw)] via-slate-900/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 mix-blend-multiply" />
                    ...
                    <div className="font-data text-[10px] tracking-widest text-accent uppercase mb-1">{place.location}</div>
                    <h4 className="font-display font-bold text-xl text-slate-900 leading-tight">{place.name}</h4>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-8">
              <div>
                <h3 className="font-display text-3xl text-slate-900 mb-5">Culinary Masterpieces</h3>
                ...
                <div className="space-y-5">
                  {d.nationalDishes?.map((dish, i) => (
                    <div key={i} className="bg-[#F4F1EA] rounded-3xl overflow-hidden flex shadow-sm items-center hover:border-accent/30 border border-transparent transition-colors">
                      <div className="w-28 h-28 shrink-0 bg-slate-200">
                        {dish.imageUrl && <img src={dish.imageUrl} alt={dish.name} className="w-full h-full object-cover" />}
                      </div>
                      <div className="p-5 flex-1">
                        <h4 className="font-display font-bold text-lg text-slate-900 mb-1">{dish.name}</h4>
                        <p className="font-editorial text-sm text-slate-600">{dish.description}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div>
                <div className="bg-accent-soft p-7 rounded-[2rem] border border-accent/20 mb-7">
                  <h3 className="font-display text-2xl text-slate-900 mb-5 flex items-center gap-2">
                    <Icon name="gem" className="w-6 h-6 text-accent" />
                    Curiosities
                  </h3>
                  <ul className="space-y-4">
                    {d.funFacts?.map((fact, i) => (
                      <li key={i} className="font-ui text-sm text-slate-800 leading-relaxed bg-white/60 p-4 rounded-2xl shadow-sm">
                        {fact}
                      </li>
                    ))}
                  </ul>
                ...
                <div>
                  <h3 className="font-display text-2xl text-slate-900 mb-5">Traveler FAQs</h3>
                  <div className="space-y-3">
                    {d.faqs?.map((faq, i) => (
                      <details key={i} className="group bg-[#F4F1EA] p-5 rounded-2xl cursor-pointer">
                        <summary className="font-ui font-bold list-none flex justify-between text-slate-800">
                          {faq.question}
                          <Icon name="chevronDown" className="w-4 h-4 text-accent group-open:rotate-180 transition-transform" />
                        </summary>
                        <p className="mt-3 font-editorial text-slate-600 text-[15px] leading-relaxed pt-3 border-t border-slate-200">
                          {faq.answer}
                        </p>
                      </details>
                    ))}
...
function SectionHeader({ title, icon }: { title: string, icon: Extract<IconName, 'geo' | 'state'> }) {
  return (
    <div className="flex items-center gap-4 mb-7">
      <span className="bg-white p-3 rounded-2xl shadow-sm text-accent">
...
function MiniRow({ label, value, suffix }: { label: string, value: string | number | undefined | null, suffix?: string }) {
  ...
  return (
    <div className="flex justify-between py-2 border-b border-slate-200 last:border-0 font-ui text-sm">
      <span className="text-slate-500">{label}</span>
      <span className="font-semibold text-slate-900 text-right max-w-[60%]">{value} {suffix}</span>
    </div>
  );
}
